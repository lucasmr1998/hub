"""Teste viabilidade por coordenadas no centro de Mococa/SP na HubSoft artelecom.
Varia o raio. Sem secrets.
"""
import sys, json
...
# centro de Mococa/SP
LAT, LNG = -21.4679, -47.0036
for raio in (250, 1000, 2000):
    payload = {'tipo_busca': 'coordenadas', 'raio': raio,
               'latitude': LAT, 'longitude': LNG, 'detalhar_portas': True}
    r = requests.post(f'{BASE}/api/v1/integracao/mapeamento/viabilidade/consultar',
                      headers={'Authorization': f'Bearer {access}', 'Content-Type': 'application/json'},
                      json=payload, timeout=45)
    print(f'\n=== Mococa centro, raio {raio}m -> HTTP {r.status_code} ===')
    try:
        data = r.json()
    except Exception:
