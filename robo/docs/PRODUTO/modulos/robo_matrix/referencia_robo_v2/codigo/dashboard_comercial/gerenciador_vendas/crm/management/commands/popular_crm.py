...
Mapeamento por prioridade (avalia na ordem abaixo e usa a primeira que bate):
  ...
  P2: Lead tem ClienteHubsoft com status_prefixo='aguardando_instalacao'
      E documentacao_validada=True
      → tipo='fechamento' (Aguardando Instalação)

  P3: Lead tem documentacao_validada=True (sem serviço aguardando_instalacao)
      → tipo='negociacao' (Proposta Enviada)
...
def determinar_tipo_estagio(lead, ids_servico_habilitado, ids_aguardando_instalacao):
    ...
    # P2: Aguardando instalação no Hubsoft + documentação validada
    if lead.pk in ids_aguardando_instalacao and getattr(lead, 'documentacao_validada', False):
        return 'fechamento'

    # P3: Documentação validada mas sem serviço aguardando → Negociação
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ----------------------------------------------------------------
        # 6. Reavaliar regras do pipeline para posicionar corretamente
        # ----------------------------------------------------------------
        if not dry_run and ops_para_criar:
            from crm.models import RegraPipelineEstagio
            if RegraPipelineEstagio.objects.filter(ativo=True).exists():
                self.stdout.write('\n  Reavaliando regras do pipeline...')
                from django.core.management import call_command
                call_command('revalidar_pipeline')
