...
from .models import (
    PipelineEstagio, RegraPipelineEstagio, EquipeVendas, PerfilVendedor,
    TagCRM, OportunidadeVenda, HistoricoPipelineEstagio,
...
class PipelineEstagioAdmin(admin.ModelAdmin):
    list_display = ['ordem', 'nome', 'pipeline_tipo', 'tipo', 'cor_badge', 'sla_horas', 'probabilidade_padrao', 'is_final_ganho', 'is_final_perdido', 'ativo']
    list_display_links = ['nome']
    list_editable = ['ordem', 'ativo']
    list_filter = ['pipeline_tipo', 'tipo', 'ativo', 'is_final_ganho', 'is_final_perdido']
    ordering = ['pipeline_tipo', 'ordem']
...
class RegraPipelineEstagioAdmin(admin.ModelAdmin):
    list_display = ['estagio', 'nome', 'ativo', 'num_condicoes', 'prioridade', 'criado_em']
    list_filter = ['estagio', 'ativo']
    search_fields = ['nome', 'estagio__nome']
    list_editable = ['ativo', 'prioridade']
    ordering = ['estagio__ordem', 'prioridade']

    def num_condicoes(self, obj):
        return len(obj.condicoes) if obj.condicoes else 0
    num_condicoes.short_description = 'Condições'
...
class OportunidadeVendaAdmin(admin.ModelAdmin):
    list_display = [
        'lead_nome', 'tipo', 'estagio', 'responsavel', 'prioridade',
        'valor_estimado', 'probabilidade', 'dias_no_estagio_display',
        'sla_badge', 'data_fechamento_previsto', 'ativo'
    ]
    list_filter = ['tipo', 'estagio', 'prioridade', 'ativo', 'origem_crm', 'responsavel']
    search_fields = ['lead__nome_razaosocial', 'lead__telefone', 'titulo']
    raw_id_fields = ['lead', 'responsavel', 'new_service', 'upgrade_plano']
    ...
    fieldsets = [
        ('Identificação', {'fields': ['lead', 'tipo', 'titulo', 'estagio', 'responsavel', 'criado_por', 'tags']}),
        ('Origem pós-venda', {'fields': ['new_service', 'upgrade_plano'], 'classes': ['collapse']}),
