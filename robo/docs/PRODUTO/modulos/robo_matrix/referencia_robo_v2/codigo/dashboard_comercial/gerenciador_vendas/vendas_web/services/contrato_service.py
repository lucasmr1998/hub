...
def _registrar_log_contrato(integracao, lead, endpoint, metodo, payload, resposta_json,
    ...
    try:
        from integracoes.models import LogIntegracao
...
# Documentos que o fluxo do robô SEMPRE coleta — o anexo/aceite só pode rodar
# com o conjunto COMPLETO. Sem isso, o retry/signal disparava no MEIO da coleta
# (ex.: só a selfie válida existia), anexava 1 arquivo e marcava como enviado —
# frente/verso chegavam depois e nunca eram anexadas (caso real: lead 243).
DESCRICOES_DOCS_OBRIGATORIAS = ('selfie_com_doc', 'frente_doc', 'verso_doc')


def docs_do_fluxo_completos(lead) -> bool:
    """True quando o lead tem UMA imagem VÁLIDA de cada documento obrigatório."""
    from vendas_web.models import ImagemLeadProspecto
    validas = set(
        lead.imagens.filter(status_validacao=ImagemLeadProspecto.STATUS_VALIDO)
        .values_list('descricao', flat=True)
    )
    return all(d in validas for d in DESCRICOES_DOCS_OBRIGATORIAS)
...
def anexar_documentos_e_aceitar_contrato(lead) -> bool:
    ...
    # GUARDA: só roda com o conjunto COMPLETO de documentos válidos — evita
    # anexo parcial no meio da coleta (e o flag travaria os que faltam).
    if not docs_do_fluxo_completos(lead):
        logger.info(
            "Lead %s ainda sem o conjunto completo de documentos válidos "
            "(selfie+frente+verso) — aguardando para anexar.", lead.pk,
        )
        return False
...
def tentar_aceite_pos_conversao(lead) -> bool:
    """Retry do anexo+aceite APÓS a conversão prospecto→cliente.

    O signal de documentos validados dispara o aceite cedo demais: naquele
    momento o cliente/serviço/contrato ainda NÃO existem no HubSoft (a
    conversão é assíncrona, via worker). Este helper roda depois da conversão:
    1. Garante o espelho local (ClienteHubsoft + serviços) — sincroniza pelo
       CPF se o lead ainda não tem cliente vinculado.
    2. Reaproveita anexar_documentos_e_aceitar_contrato (idempotente).

    Só age se a documentação do lead está validada e ainda não anexamos.
    """
    try:
        if lead.anexos_contrato_enviados:
            return True
        if not docs_do_fluxo_completos(lead):
            logger.info("[ACEITE-POS] Lead %s sem o conjunto completo de docs "
                        "válidos (selfie+frente+verso) — pulando.", lead.pk)
            return False
        # Garante o espelho local do cliente/serviços
        if not lead.clientes_hubsoft.exists():
            from integracoes.models import IntegracaoAPI
            from integracoes.services.hubsoft import HubsoftService
            ...
            if integ and lead.cpf_cnpj:
                ...
                if clientes:
                    service._sincronizar_dados_cliente(clientes[0], lead)  # noqa: SLF001
        ok = anexar_documentos_e_aceitar_contrato(lead)
        logger.info("[ACEITE-POS] Lead %s — anexo+aceite pós-conversão: %s", lead.pk, ok)
        return ok
    except Exception as exc:  # noqa: BLE001
        logger.error("[ACEITE-POS] Lead %s falhou: %s", lead.pk, exc)
        return False
