...
class HistoricoStatusLeadInline(admin.TabularInline):
    model = HistoricoStatusLead
    fields = ('status_anterior', 'status_novo', 'origem', 'alterado_por', 'observacao', 'data_alteracao')
    readonly_fields = ('status_anterior', 'status_novo', 'origem', 'alterado_por', 'observacao', 'data_alteracao')
    extra = 0
    can_delete = False
    ordering = ['-data_alteracao']
    verbose_name = "Mudança de Status"
    verbose_name_plural = "Histórico de Status (status_api)"
...
class LeadProspectoAdmin(admin.ModelAdmin):
    ...
    list_per_page = 25
    inlines = [ProspectoInline, HistoricoContatoInline, HistoricoStatusLeadInline, AtendimentoFluxoInline, DocumentoLeadInline, ImagemLeadInline]
    save_on_top = True
...
@admin.register(HistoricoStatusLead)
class HistoricoStatusLeadAdmin(admin.ModelAdmin):
    list_display = ['lead', 'status_anterior', 'status_novo', 'origem', 'alterado_por', 'data_alteracao']
    list_filter = ['origem', 'status_novo', 'data_alteracao']
    search_fields = ['lead__nome_razaosocial', 'lead__cpf_cnpj', 'status_anterior', 'status_novo']
    readonly_fields = ['lead', 'status_anterior', 'status_novo', 'origem', 'alterado_por',
                       'observacao', 'data_alteracao']
    ordering = ['-data_alteracao']
...
@admin.register(UpgradePlano)
class UpgradePlanoAdmin(admin.ModelAdmin):
    """Visão dos pedidos de upgrade de plano (solicitados via fluxo
    conversacional) e do status de processamento no HubSoft pelo webdriver."""
    ...
    list_display = [
        ...
        'situacao_webdriver',
        'criado_em',
        'hubsoft_processado_em',
    ]
    list_display_links = ['id', 'lead_link']
    list_filter = [
        'status',
        'hubsoft_processado_status',
        'criado_em',
        'hubsoft_processado_em',
    ]
    search_fields = [
    ...
    readonly_fields = [
        ...
        'hubsoft_processado_em', 'hubsoft_processado_status', 'hubsoft_erro',
    ]
    fieldsets = (
        ('Solicitação', {
            'fields': ('lead', 'status', 'id_cliente_servico', 'id_plano_novo', 'observacoes')
        }),
        ('Processamento no HubSoft (preenchido pelo webdriver)', {
            'fields': ('hubsoft_processado_em', 'hubsoft_processado_status', 'hubsoft_erro')
        }),
        ('Auditoria', {
            'fields': ('criado_em', 'atualizado_em', 'finalizado_em'),
            'classes': ('collapse',),
        }),
    )
    save_on_top = True

    @admin.display(description='Lead', ordering='lead')
    def lead_link(self, obj):
        if not obj.lead_id:
            return '—'
        nome = getattr(obj.lead, 'nome_razaosocial', None) or f'Lead {obj.lead_id}'
        return format_html(
            '<a href="/admin/vendas_web/leadprospecto/{}/change/">{}</a>',
            obj.lead_id, nome,
        )
    ...
    def status_badge(self, obj):
        cores = {
            'em_coleta': '#6c757d',   # cinza
            'finalizado': '#0d6efd',  # azul (pronto pro webdriver)
    ...
    @admin.display(description='Webdriver / HubSoft')
    def situacao_webdriver(self, obj):
        if obj.hubsoft_processado_em:
            cores = {'sucesso': '#198754', 'falha': '#dc3545', 'dry_run': '#fd7e14'}
            cor = cores.get(obj.hubsoft_processado_status, '#6c757d')
            rotulo = obj.get_hubsoft_processado_status_display() or 'Processado'
            return format_html(
                '<span style="background:{};color:#fff;padding:2px 8px;'
                'border-radius:10px;font-size:11px;font-weight:600;">{}</span>',
                cor, rotulo,
        ...
        if obj.status == 'finalizado':
            return format_html(
                ...
                'border-radius:10px;font-size:11px;font-weight:600;">Pendente</span>'
            )
...
class BairroViabilidadeInline(admin.TabularInline):
    model = BairroViabilidade
    fields = ('nome', 'cep', 'ativo', 'observacao')
    extra = 1
    verbose_name = 'Bairro atendido'
    verbose_name_plural = '🏘️ Bairros atendidos nesta cidade'
    classes = ('collapse',)
...
class CidadeViabilidadeAdmin(admin.ModelAdmin):
    list_display = (
        'cidade_estado_badge', 'regional_badge', 'cep_display', 'bairros_display',
        'observacao_curta', 'ativo_badge', 'data_criacao',
    )
    list_filter = ('atende_cidade_inteira', 'regional', 'estado', 'ativo', 'data_criacao')
    search_fields = ('cidade', 'cep', 'estado', 'regional', 'bairros__nome')
    ordering = ('regional', 'estado', 'cidade', 'cep')
    list_per_page = 50
    readonly_fields = ('data_criacao', 'data_atualizacao')
    inlines = [BairroViabilidadeInline]

    def get_queryset(self, request):
        return super().get_queryset(request).prefetch_related('bairros')

    fieldsets = (
        ('📍 Localização', {
            'fields': ('cidade', 'estado', 'regional', 'cep', 'atende_cidade_inteira'),
            'description': (
                'Informe a cidade e estado (obrigatório). '
                'O CEP é opcional — se não informado, toda a cidade é '
                'considerada com viabilidade. Marque "Atende a cidade inteira" '
                'se não quiser restringir por bairro; caso contrário, cadastre '
                'os bairros atendidos na lista abaixo.'
            ),
        }),
        ('📝 Detalhes', {
    ...
    regional_badge.short_description = 'Regional'
    regional_badge.admin_order_field = 'regional'
    ...
    def bairros_display(self, obj):
        if obj.atende_cidade_inteira:
            return format_html(
                '<span style="background:#dcfce7;color:#166534;padding:2px 8px;'
                'border-radius:10px;font-size:11px;font-weight:700;">🏙️ Cidade inteira</span>'
            )
        bairros = [b for b in obj.bairros.all() if b.ativo]
        if not bairros:
            return format_html(
                '<span style="color:#aaa;font-size:11px;">Nenhum bairro cadastrado</span>'
            )
        chips = format_html_join(
            ' ',
            '<span style="background:#e0e7ff;color:#3730a3;padding:2px 8px;'
            'border-radius:10px;font-size:11px;font-weight:600;white-space:nowrap;">{}</span>',
            ((b.nome,) for b in bairros[:6]),
        )
        extra = len(bairros) - 6
        if extra > 0:
            chips = format_html(
        ...
        return chips
    bairros_display.short_description = 'Bairros atendidos'
...
#  NEW SERVICE — admin
# ════════════════════════════════════════════════════════════════════
class ImagemNewServiceInline(admin.TabularInline):
    model = ImagemNewService
    fields = ('link_url', 'descricao', 'status_validacao', 'preview_imagem', 'data_criacao')
    readonly_fields = ('preview_imagem', 'data_criacao')
    extra = 0
    ordering = ['-data_criacao']
    ...
    def preview_imagem(self, obj):
        if obj.link_url:
            return format_html(
                ...
                '<img src="{u}" style="max-height:80px;max-width:120px;border-radius:6px;border:1px solid #ddd;" />'
                '</a>',
                u=obj.link_url,
            )
        return '-'
...
class NewServiceAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'lead', 'status', 'matrix_sync_status', 'tipo_imovel',
        'cidade', 'id_plano_rp', 'data_instalacao', 'criado_em', 'finalizado_em',
    )
    list_filter = (
        'status', 'matrix_sync_status', 'tipo_imovel', 'tipo_residencia',
        'turno_instalacao',
    )
    search_fields = (
        'lead__nome_razaosocial', 'lead__cpf_cnpj', 'lead__telefone',
        'cep', 'bairro', 'cidade',
        'id_atendimento_matrix', 'id_os_matrix',
    )
    readonly_fields = (
        'criado_em', 'atualizado_em', 'finalizado_em',
        'id_atendimento_matrix', 'id_os_matrix', 'id_cliente_servico_origem',
        'data_sync_matrix', 'tentativas_sync_matrix',
    )
    date_hierarchy = 'criado_em'
    inlines = [ImagemNewServiceInline]
    fieldsets = (
        ('Vínculo', {'fields': ('lead', 'status')}),
        ('Imóvel', {'fields': ('tipo_imovel', 'tipo_residencia')}),
        ('Endereço da nova instalação', {
            'fields': ('cep', 'rua', 'numero_residencia', 'bairro', 'cidade', 'estado',
                       'endereco_confirmado', 'ponto_referencia'),
        }),
        ('Plano', {'fields': ('id_plano_rp', 'plano_confirmado', 'id_dia_vencimento', 'valor')}),
        ('Confirmação / Ajuste', {'fields': ('dados_confirmados', 'tipo_ajuste')}),
        ('Documentação', {
            'fields': ('doc_selfie_recebida', 'doc_frente_recebida', 'doc_verso_recebida'),
        }),
        ('Agendamento', {'fields': ('turno_instalacao', 'data_instalacao')}),
        ('Sincronização Matrix (Atendimento + OS)', {
            'fields': (
                'matrix_sync_status',
                ('id_atendimento_matrix', 'id_os_matrix'),
                'id_cliente_servico_origem',
                'data_sync_matrix', 'tentativas_sync_matrix',
                'ultimo_erro_sync_matrix',
            ),
        }),
        ('Auditoria', {
            'fields': ('criado_em', 'atualizado_em', 'finalizado_em', 'observacoes'),
        }),
    )


@admin.register(ImagemNewService)
class ImagemNewServiceAdmin(admin.ModelAdmin):
    list_display = ('id', 'new_service', 'descricao', 'status_validacao', 'data_criacao')
    list_filter = ('status_validacao', 'descricao')
    search_fields = ('new_service__lead__nome_razaosocial', 'descricao')
    readonly_fields = ('data_criacao',)
