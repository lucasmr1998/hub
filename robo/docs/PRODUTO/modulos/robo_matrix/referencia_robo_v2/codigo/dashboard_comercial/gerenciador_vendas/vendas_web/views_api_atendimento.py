...
from .models import (
    ...
    TentativaResposta, LeadProspecto, HistoricoContato, LogSistema
)
...
def responder_questao_n8n(request, atendimento_id):
    ...
    try:
        ...
        if hasattr(atendimento, 'responder_questao_inteligente'):
            ...
            # dados_extras pode conter o objeto QuestaoFluxo cru em
            # 'proxima_questao' (não serializável) — converte pra dict.
            if isinstance(dados_extras, dict) and dados_extras.get('proxima_questao') is not None:
                pq = dados_extras['proxima_questao']
                if hasattr(pq, '_meta'):  # é uma instância de model
                    dados_extras = {**dados_extras, 'proxima_questao': _serialize_questao_fluxo(pq)}
...
# NOTE: a definição completa de _serialize_questao_fluxo (com
# incluir_detalhes_inteligentes) está no topo do arquivo. Uma 2ª
# definição reduzida existia aqui e sombreava a completa — removida
# porque quebrava obter_questao_inteligente_n8n (TypeError no kwarg).
...
def obter_questao_inteligente_n8n(request, fluxo_id, indice_questao):
    ...
    try:
        ...
        # Aceita atendimento_id solto na query (atalho do canal)
        if not contexto.get('atendimento_id') and request.GET.get('atendimento_id'):
            try:
                contexto['atendimento_id'] = int(request.GET.get('atendimento_id'))
            except (TypeError, ValueError):
                pass

        # Fluxo de upgrade: deriva lead_id e id_servico_hubsoft_atual do
        # próprio atendimento (canal só precisa mandar atendimento_id).
        if contexto.get('atendimento_id'):
            try:
                atendimento_ctx = AtendimentoFluxo.objects.get(
                    id=contexto['atendimento_id'])
                from vendas_web.services.upgrade_plano_service import enriquecer_contexto_upgrade
                contexto = enriquecer_contexto_upgrade(atendimento_ctx, contexto)
            except AtendimentoFluxo.DoesNotExist:
                pass
            except Exception:
                pass
