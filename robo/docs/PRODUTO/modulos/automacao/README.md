...
| `definir_propriedade_lead` | Comercial › Leads | Definir propriedade do lead | ✅ espelho do de oportunidade; registry `propriedades_lead.py` (13 campos em dropdown, incl. `dado_custom` com chave); a `propriedade` aceita `{{...}}` — no bot de venda vem de `{{nodes.validar.chave}}`, porque o fluxo não sabe em tempo de desenho qual campo escrever; chave que não é campo do lead vira **skip**, não erro; `somente_se_vazio` ligado por padrão (repergunta não apaga o que um humano corrigiu) |
...
| `checklist_proximo_item` | Comercial › Checklist | Checklist: próximo item | ✅ ponte checklist→fluxo (bot de vendas vira grafo); acha o próximo item pendente via `services/checklist.proximo_item`; saídas `tem_item`/`completo`/`erro`; sem IA; **renderiza `{campo}`** da pergunta a partir da ficha do lead (ex: confirmação de endereço com `{rua}`/`{cidade}`, `{nome}` na msg de plano); placeholder sem campo some, `{{...}}` da engine não é tocado |
...
| `checklist_respostas` | Comercial › Checklist | Checklist: respostas | ✅ devolve o que a entidade JÁ respondeu como dicionário (`{{nodes.<h>.cep}}`); peça que faltava pra um nó adiante usar resposta de turno ANTERIOR (o grafo só enxergava a resposta corrente e a contagem); saídas `sucesso`/`vazio`/`erro`; sem IA |
| `viabilidade_consultar` | Comercial › Viabilidade | Viabilidade: consultar cobertura | ✅ embrulha `apps.comercial.viabilidade.services.consultar_viabilidade` (HubSoft → `CidadeViabilidade` local → ViaCEP pro que faltar), **não** o nó HubSoft cru: a resposta da API traz `projetos` como LISTA quando há projeto e como TEXTO quando não há, então teste de vazio dá "tem cobertura" nos dois casos; o service ainda confere PORTAS LIVRES e separa "não atende" de "não sei"; saídas `cobertura_ok`/`fora_cobertura`/`pendente_revisao`/`erro` |
| `viacep` | Comercial › Endereço | ViaCEP: buscar endereço | ✅ embrulha `buscar_endereco_por_cep` (ViaCEP); traz logradouro/bairro/cidade/uf pro fluxo; opção `gravar_no_lead` persiste na ficha (rua/bairro/cidade/estado, sem sobrescrever), pra confirmação num turno seguinte já achar o endereço; saídas `encontrado`/`nao_encontrado`/`erro` (CEP errado é `nao_encontrado`, não erro) |
