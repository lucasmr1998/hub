...
- **Fase 3 (preparada em DEV, branch `aposentar-marketing-fase3`, NÃO deployada):** deletado todo o código do motor antigo (engine, views, signals, admin, urls, templates Drawflow, cron + commands, ferramentas de migração `migrar/cutover/migracao_marketing`, testes antigos). O app virou **husk**: `models.py` vazio + `apps.py` + migrations `0001-0006` (tombstone) + **`0007` DeleteModel**. `0007` aplicada em `aurora_dev` → **8 tabelas dropadas**; `check` limpo, collection limpa (1196), engine nova verde (35 testes), grafo coerente (`emails/0001` ↔ `automacoes/0004` tombstone — por isso husk, não remoção total). Bônus: corrigida regressão do refactor de memória no `ia_agente` (`getattr`) + 2 testes atualizados pro modelo novo.
- **Runbook de DEPLOY da Fase 3 (prod, gated — merge a branch só após isto):** (1) Fases 1-2 deployadas + **verificadas em prod** (evento real dispara a engine nova 1×; timeline do CRM carrega); (2) **backup** antes do drop — `pg_dump --table='automacoes_*'` via `.env.prod_readonly`, offline (dumpdata não serve mais — app é husk); (3) merge + deploy (a `0007` dropa as 8 tabelas em prod); (4) confirmar `\dt automacoes_*` vazio em prod.
- **Fase 4 (cleanup, deferido):** permissão órfã `marketing.gerenciar_automacoes` na matriz; drift pré-existente de `notificacoes` (não meu).
