...
- **Output:** `manage.py check` limpo; 11 testes dos 2 nodes novos + 40 dos HubSoft. Catálogo do README atualizado. Motor antigo **intocado**.
- **Status:** Fase 1 **in progress**. Feito: 2 nodes locais + retrofit do picker HubSoft. Faltam **3 nodes** (`gerar_contrato_hubsoft`, `assinar_contrato_hubsoft` com picker; `enviar_venda_whatsapp` com picker uazapi). Depois: Fase 2 (tradutor + shadow + comparador).
